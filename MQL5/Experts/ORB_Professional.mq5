...
// Cached market data (optimization)
...
void OnTick()
{
    //--- Update market data cache (Optimize: calculate once per tick)
    UpdateMarketData();
...
//+------------------------------------------------------------------+
//| Update cached market data for current tick                       |
//+------------------------------------------------------------------+
void UpdateMarketData()
{
    //--- Get current time structure once
    TimeToStruct(TimeCurrent(), g_currentTime);

    //--- Get current tick data once
    if(!SymbolInfoTick(_Symbol, g_currentTick))
    {
        Print("ERROR: Failed to update cached tick data");
        // We can't return here as we need to update other caches,
        // but trading functions should check for valid data
    }
    ...
    if(CopyBuffer(g_atrHandle, 0, 0, 1, buffer) == 1)
        g_currentATR = buffer[0];
    else
    {
        g_currentATR = 0;
        Print("ERROR: Failed to update cached ATR value");
    }

    //--- Get ADX value once if needed
    if(InpOnlyTrending)
    {
        if(CopyBuffer(g_adxHandle, 0, 0, 1, buffer) == 1)
            g_currentADX = buffer[0];
        else
        {
            g_currentADX = 0;
            Print("ERROR: Failed to update cached ADX value");
        }
    }
}
...
void CheckNewDay()
{
    ...
    // Optimized: use cached time structure
    TimeToStruct(g_lastDayChecked, lastTime);

    if(g_currentTime.day != lastTime.day)
    {
...
{
    // Optimized: use cached time structure
    int currentHour = g_currentTime.hour;
    int currentMinute = g_currentTime.min;
...
void CalculateORB(SORBSession &session)
{
    ...
    //--- Get Highs and Lows for ORB period (Optimized: using CopyHigh/CopyLow instead of CopyRates)
    double highs[], lows[];
    ArraySetAsSeries(highs, true);
    ArraySetAsSeries(lows, true);

    //--- Only copy needed data (High and Low)
    int copiedHigh = CopyHigh(_Symbol, PERIOD_M1, 0, InpORBMinutes, highs);
    int copiedLow = CopyLow(_Symbol, PERIOD_M1, 0, InpORBMinutes, lows);

    if(copiedHigh < InpORBMinutes || copiedLow < InpORBMinutes)
    {
        Print("ERROR: Failed to copy M1 bars for ", session.name, " ORB calculation");
        return;
    }

    //--- Find high and low using native array functions (Optimized: O(N) internal search)
    int highIdx = ArrayMaximum(highs);
    int lowIdx = ArrayMinimum(lows);

    if(highIdx == -1 || lowIdx == -1)
    {
        Print("ERROR: Failed to find min/max in ORB calculation");
        return;
    }

    session.high = highs[highIdx];
    session.low = lows[lowIdx];
...
void CheckSessionSignal(SORBSession &session)
{
    ...
    //--- Get current price
    double ask = g_currentTick.ask;
    double bid = g_currentTick.bid;
    double point = g_symbolPoint;
...
bool PassesFilters()
{
    ...
    if(InpMaxSpread > 0)
    {
        // Optimized: calculate spread from cached tick data
        int spread = (int)MathRound((g_currentTick.ask - g_currentTick.bid) / g_symbolPoint);
...
double GetATR()
{
    // Optimized: return cached value calculated once per tick
    return g_currentATR;
}
